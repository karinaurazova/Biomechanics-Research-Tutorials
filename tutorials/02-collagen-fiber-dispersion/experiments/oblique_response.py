"""Compare dispersed-fiber responses when the mean is oblique to loading."""
...
def main() -> None:
    """Save oblique-loading response curves in both languages."""
    stretches = np.linspace(1.0, 1.25, 121)
    concentrations = (0.0, 1.0, 4.0, 12.0)
    mean = np.deg2rad(45.0)
    ...
    base_output = Path(__file__).resolve().parents[1] / "figures" / "oblique_response.png"
    ...
    for language in SUPPORTED_LANGUAGES:
        ...
        visualization.save_response_curves(
            ...
            mean,
            output,
            title_key="oblique_response_title",
            language=language,
        )
