# Tutorial 24 master class — Physics-Informed Learning for Soft-Tissue Mechanics

## Purpose

This 120-minute master class turns Tutorial 24 into a guided teaching session on physics-informed learning with sparse data, boundary terms and equilibrium residuals.  The session uses only the repository code, synthetic data and saved visualizations, so participants can reproduce every result after the workshop.
...
1. explain the modelling question addressed by Tutorial 24;
...
- compare data-only, physics-only and data+physics solutions.
- sweep the PDE-loss weight and inspect residual/error trade-offs.
- interpret inverse stiffness-scale calibration and curriculum training.
...
3. Run `python tutorials/24-physics-informed-learning-soft-tissue-mechanics/reproduce.py`.
