# Tutorial 20 master class — Multimodal Verification-Ready Synthetic Benchmark

## Purpose

This 120-minute master class turns Tutorial 20 into a guided teaching session on a full verification-ready multimodal benchmark from hidden ground truth to inverse material identification.  The session uses only the repository code, synthetic data and saved visualizations, so participants can reproduce every result after the workshop.
...
1. explain the modelling question addressed by Tutorial 20;
...
- run the complete synthetic pipeline from microstructure to inverse parameter recovery.
- compare oracle, SEM-only, polarization-only and multimodal-fusion scenarios.
- build an error budget across segmentation, orientation, mechanics and inverse stages.
...
3. Run `python tutorials/20-multimodal-verification-ready-benchmark/reproduce.py`.
