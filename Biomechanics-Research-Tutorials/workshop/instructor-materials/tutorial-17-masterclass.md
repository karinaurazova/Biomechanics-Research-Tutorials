# Tutorial 17 master class — Microstructure Segmentation: Classical Methods, SAM and μSAM

## Purpose

This 120-minute master class turns Tutorial 17 into a guided teaching session on comparison of classical, learnable and prompt-based microstructure segmentation under topology-aware metrics.  The session uses only the repository code, synthetic data and saved visualizations, so participants can reproduce every result after the workshop.
...
1. explain the modelling question addressed by Tutorial 17;
...
- run the segmentation benchmark and rank methods by Dice, topology error and fibre continuity.
- compare point, box and automatic-prompt abstractions instead of judging only by visual appearance.
- discuss why a mask with good overlap may still fail for downstream mechanics.
...
3. Run `python tutorials/17-microstructure-segmentation-sam-usam/reproduce.py`.
