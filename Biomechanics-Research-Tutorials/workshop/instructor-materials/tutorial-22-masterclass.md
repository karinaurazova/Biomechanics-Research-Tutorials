# Tutorial 22 master class — RVE Homogenization

## Purpose

This 120-minute master class turns Tutorial 22 into a guided teaching session on RVE homogenization of heterogeneous fibre networks into an effective stiffness tensor.  The session uses only the repository code, synthetic data and saved visualizations, so participants can reproduce every result after the workshop.
...
1. explain the modelling question addressed by Tutorial 22;
...
- compute Voigt, Reuss and periodic homogenized stiffness estimates.
- check Hill–Mandel consistency and directional stiffness.
- run a small RVE resolution study and interpret convergence.
...
3. Run `python tutorials/22-rve-homogenization/reproduce.py`.
