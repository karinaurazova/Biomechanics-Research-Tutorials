# Tutorial 23 master class — Surrogate Modeling for Homogenized Tissue Mechanics

## Purpose

This 120-minute master class turns Tutorial 23 into a guided teaching session on surrogate modelling for fast mechanics-aware prediction of homogenized tissue response.  The session uses only the repository code, synthetic data and saved visualizations, so participants can reproduce every result after the workshop.
...
1. explain the modelling question addressed by Tutorial 23;
...
- train linear, quadratic and random-feature surrogates.
- compare parity plots, extrapolation maps and learning curves.
- use ensemble uncertainty to select new simulation points.
...
3. Run `python tutorials/23-surrogate-modeling-homogenized-tissue-mechanics/reproduce.py`.
