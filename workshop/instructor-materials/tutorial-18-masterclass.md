# Tutorial 18 master class — Orientation Distributions and Concentration Parameters

## Purpose

This 120-minute master class turns Tutorial 18 into a guided teaching session on recovery of orientation distributions and concentration parameters from several image-derived representations.  The session uses only the repository code, synthetic data and saved visualizations, so participants can reproduce every result after the workshop.
...
1. explain the modelling question addressed by Tutorial 18;
...
- recover ODFs from ground truth, SEM-like, polarization-like and segmented-mask modalities.
- compare axial mean direction, concentration, entropy and orientation-tensor anisotropy.
- trace how orientation errors propagate into a simple stiffness index.
...
3. Run `python tutorials/18-orientation-distributions-concentration/reproduce.py`.
